...
Task tool:
  ...
  prompt: |
    ...
    **截图策略**: 不要对每个步骤都截图。仅在以下情况截图:
    - 发现 Bug 时（作为 Bug 报告的证据）
    - 测试失败时（记录失败现场）
    正常通过的测试步骤用文字记录即可，不需要截图。
...
Task tool:
  ...
  prompt: |
    ...
    **截图策略**: 仅在发现 Bug 或验证失败时截图，正常通过不截图。
...
- **测试证据策略** — API 请求/响应用文字记录到 log.md；浏览器截图仅在发现 Bug 或测试失败时保留，正常通过的步骤不截图

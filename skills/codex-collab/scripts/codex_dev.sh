...
# 执行命令。默认走受控的 full-auto（workspace-write + 不询问审批），只能在工作目录内读写、
# 不能联网；需要联网装依赖 / 写工作目录外文件时才显式加 --yolo 放开全部沙箱。
#
# 用法：
#   首次任务：codex_dev.sh -d DIR [-m MODEL] [--add-dir DIR]... [--yolo] [-l LABEL] -- <任务描述>
...
#                  强烈建议是 git 仓库根，便于事后 `git diff` 审查 codex 的改动。resume 时被忽略
#   -t THREAD_ID   续接已有任务会话；不传则新建
#   -m MODEL       codex 模型（透传 -m），默认 codex 自选
#   --add-dir DIR  额外可写目录（透传 --add-dir，可重复）；必须为已存在的绝对路径。yolo 下无意义会忽略
#   --yolo         放开全部沙箱与审批 = codex exec --dangerously-bypass-approvals-and-sandbox。
#                  codex 可联网、可写工作目录以外的任意文件。**危险**，仅在确需装依赖 /
#                  跨目录写时使用；不带此 flag 时默认 full-auto（workspace-write + never approval）。
#                  resume 时被忽略（权限由原会话继承）
...
#   * resume 时不传 --cd / 权限 flag（codex 会拒绝），由原会话继承；检测到会 stderr 警告并忽略
#   * codex --json 的 stdout 是纯 JSONL；"Reading additional input from stdin..." 之类提示走 stderr，不进 jq
#   * ⚠️ 干完活后 Claude 应回到工作目录 `git diff` / `git status` 审查 codex 的改动，别盲信
#
# 退出码：
#   0  成功
#   2  参数错误
...
while [[ $# -gt 0 ]]; do
  case "$1" in
    ...
    --add-dir) require_arg "$1" "${2-__MISSING__}" || exit 2; ADD_DIRS+=("$2"); shift 2 ;;
    --yolo) YOLO=1; shift ;;
    -h|--help) usage; exit 0 ;;
    --) shift; PROMPT="$*"; break ;;
...
5. 只在工作目录范围内、只围绕本任务改动，不要顺手做无关重构。
...
if [[ -n "$THREAD_ID" ]]; then
  # resume：codex 不接受 --cd / 沙箱 / 审批 flag（由原会话继承），只允许 --json / -m 等
  [[ -n "$DIR" ]]           && echo "warn: resume 模式下 -d/--cd 由原会话决定，已忽略" >&2
  [[ "$YOLO" == "1" ]]      && echo "warn: resume 模式下 --yolo 由原会话决定，已忽略" >&2
  [[ ${#ADD_DIRS[@]} -gt 0 ]] && echo "warn: resume 模式下 --add-dir 由原会话决定，已忽略" >&2
  ...
  CODEX_ARGS=(exec --json --cd "$DIR")
  if [[ "$YOLO" == "1" ]]; then
    echo "⚠️  [codex_dev] --yolo 已启用：codex 将放开全部沙箱与审批，可联网、可写工作目录以外任意文件。请确保任务可信、目录可回滚（git）。" >&2
    CODEX_ARGS+=(--dangerously-bypass-approvals-and-sandbox)
    [[ ${#ADD_DIRS[@]} -gt 0 ]] && echo "warn: --yolo 下 --add-dir 无意义（已全盘可写），忽略" >&2
  else
    # 默认 full-auto：codex exec 非交互本就不询问审批（命令失败直接返回模型，不升级），
    # 只需给 workspace-write 即可写工作目录内文件；exec 不接受 --ask-for-approval，别传。
    CODEX_ARGS+=(--sandbox workspace-write)
    # 注意：macOS 自带 bash 3.2，set -u 下直接展开空数组 "${ADD_DIRS[@]}" 会报 unbound，先守卫元素数
    if [[ ${#ADD_DIRS[@]} -gt 0 ]]; then
      for d in "${ADD_DIRS[@]}"; do
        if [[ "$d" != /* || ! -d "$d" ]]; then
          echo "error: --add-dir 必须是已存在的绝对路径: $d" >&2
          exit 2
        fi
